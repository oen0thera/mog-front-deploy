...
export default function SelectMainpage({
  ...
  const routineSetting = async e => {
    ...
    if (e.target.id === 'reName') {
      ...
      await axios.delete(
        `${URL.REALDATA}/${parseInt(useDataRoutine[getSelectRoutine - 1].realRoutineId)}/delete`,
        {
          headers: {
    ...
    <>
      <div className={styles.mainContainer}>
          ...
          {checkRoutineUser !== undefined ? (
            checkRoutineUser.length > 0 ? (
              checkRoutineUser.map((item, index) => (
                <Button
                  key={index}
                  className={`${styles.prettyButton} mb-3`}
                  style={{
                    height: '150px',
                    width: '95%',
                    display: 'flex',
              ...
              <div>
                <h1 className={styles.prettyText}>
                  루틴이 없습니다.
                  <br />
                  자신만의 루틴을 만들어 보세요!
                </h1>
        ...
        {startRrcodResultData === false ? (
          <footer className={`${styles.flexButton}`}>
            <Button
              ...
              type="button"
              onClick={e => navigate('/data/select', { state: true })}
            >
              루틴 생성
            </Button>
          </footer>
        ...
        <Modal
          className={styles.ModelContainer}
          show={isShowRename}
          onHide={() => setSIsShowRename(false)}
        >
          <Modal.Header>수정할 이름 입력</Modal.Header>
          <Modal.Body>
            <input style={{ width: '100%' }} type="text" ref={inputRef} />
            <div
              style={{
                display: 'flex',
