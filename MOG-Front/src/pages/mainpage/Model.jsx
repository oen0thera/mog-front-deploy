...
function Model({
  ...
  const loadRoutineDetail = () => {
    ...
    startTimer();
    if (!checkPageWhenRunningPage) {
      navigate(`/data/runningroutine?routineId=${routineId}&DetailId=1`, {
  ...
  const stopRrcodResultData = () => {
    if (!resetTimeCheckBoolean) {
      ...
      const allSetNum = resultData.routineEndDetails.reduce(
        (index, item) => index + parseInt(item.setNumber),
        0,
      );
      const allReps = resultData.routineEndDetails.reduce(
        (index, item) => index + parseInt(item.reps),
        0,
      );
      const allWeight = resultData.routineEndDetails.reduce(
        (index, item) => index + parseInt(item.weight),
        0,
      );
      const inputResult = {
        ...resultData,
  ...
  useEffect(() => {
    axios.get(URL.ROUTINERESULT).then(res => setRoutineLength(res.data.length));
  }, []);
  return (
    <>
        ...
        <div className="modal-dialog">
          <div className="modal-content" style={{ backgroundColor: 'white' }}>
            ...
            <div className="modal-body">
              <p>
                {startRrcodResultData ? '운동을 종료 하시겠습니까?' : '운동을 시작하시겠습니까?'}
              </p>
